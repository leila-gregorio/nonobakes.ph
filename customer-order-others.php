    ...
    <section class="orders">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>"   method="POST">
            <div class="radio-cont animate-left">
                ...
                <?php
                     // LOAD DYNAMIC ADD ONS
                    $sql = "SELECT * FROM `product` WHERE `productStatus`='ACTIVE' and `productDescription`='OTHERS'";
                    if ($stmt = mysqli_prepare($link, $sql)) {
                    if (mysqli_stmt_execute($stmt)) {
                    $result = mysqli_stmt_get_result($stmt);
                    if (mysqli_num_rows($result) > 0) {
                    echo "<h2 class='section-header description-title'>Non Bento Cake Menu</h2><hr class='hr-style' style='width:90%; margin: 10px;'>";
                    echo "<br>";
            ...
            <div class="container content-section">
                <h2 class="section-header description-title">ORDER</h2><hr class="hr-style" style="width:90%; margin: 10px;">
                <div class="cart-row">
   ...
   <?php
                    ...
                    if(isset($items)) {
                        ...
                        <div class="cart-row">
                            ...
                            <div class="cart-quantity cart-column counter">
                                <input class="cart-quantity-input" name="cartQuantity[]" id="disable" type="number" max="<?php if(isset($_GET['slots'])){echo $_GET['slots'];}?>"value="0" >
                            </div></div>
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if (mysqli_num_rows($result) < 1 ) {
                ...
                if ($stmt = mysqli_prepare($link, $sql)) {
                    ...
                    if (mysqli_stmt_execute($stmt)) {
                        echo "<script type='text/javascript'> alert('Successfully Added');window.location='customer-payment-details.php'</script>";
                    } else {
                ...
                if($stmt = mysqli_prepare($link, $sql)){
                    ...
                    if(mysqli_stmt_execute($stmt)){
                        echo "<script type='text/javascript'> alert('Successfully Edited');window.location='customer-payment-details.php'</script>";
                    }
                    else{
