...
if(isset($_GET['id']) && !empty(trim($_GET['id']))){
    $sql = "SELECT fontStyle  FROM `form` WHERE formID = ?";
...
if(isset($_POST['btnupdate'])){
    ...
    $sql = "UPDATE form SET fontStyle = ? WHERE formID = ?; ";
    if($stmt = mysqli_prepare($link, $sql)){
        mysqli_stmt_bind_param($stmt, "ss",$_POST['editCakeFont'] ,$_GET['id']);
        if(mysqli_stmt_execute($stmt)){
                    ...
                    <div class="row ">
                        <form action = "<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method = "POST">
                            <div class="form-group cont-edit center-item">
                                    <label for="editCakeFont">Cake Font</label> <br>
                                    <input type="text" class=" form-control" name="editCakeFont" value="<?php echo $row['fontStyle']; ?>"/><br><br>
