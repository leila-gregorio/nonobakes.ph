...
if(isset($_GET['id']) && !empty(trim($_GET['id']))){
    $sql = "SELECT cakeFilling  FROM `form` WHERE formID = ?";
...
if(isset($_POST['btnupdate'])){
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        mysqli_stmt_bind_param($stmt, "ss",$_POST['editCakeFilling'] ,$_GET['id']);
        if(mysqli_stmt_execute($stmt)){
...
<html>
<head>
    <title>Edit Cake Filling</title>
    <!-- icons -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.14.0/css/all.min.css">
</head>
<body>
<div class="modal-style modal-header" id="mod2">
                    ...
                    <div class="row ">
                        <form action = "<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method = "POST">
                            <div class="form-group cont-edit center-item">
                                <label for="editCakeFilling">Cake Filling</label>

                                <input type="text" class="form-control" name="editCakeFilling" value="<?php echo $row['cakeFilling']; ?>"/><br><br>
